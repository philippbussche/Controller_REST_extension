...
import com.appdynamics.ace.extension.groovy.command.api.*;
...
public class GroovyConsoleService {
    ...
    public GroovyConsoleService(String path) throws Exception {
        ...
        _gse.getConfig().setScriptBaseClass(BaseGroovyScript.class.getName());
        _gse.getGroovyClassLoader().addClasspath(findGroovyExtensionClasspath());


    }

    private String findGroovyExtensionClasspath() {

        String[] l = getExtensionDir().list(new FilenameFilter() {
            @Override
            public boolean accept(File dir, String name) {
                return name.startsWith("ace_Rest_groovy") && name.endsWith(".jar");
            }
        });

        //TODO :
        if (l != null && l.length>0) return new File(getExtensionDir(),l[0]).getAbsolutePath();
        else return "";
    }
    ...
    @DELETE
    @Path("script/{name}")
    public String deleteScript(@PathParam("name")String name) {
        try {
    ...
    public ScriptResult executeScript(@PathParam("name") String scriptname) {
        try {
            ...
            Script originalScript =  _gse.createScript(scriptname + "." + GROOVY_POSTFIX, binding);
            attachScriptConsole(originalScript);
    ...
    Script createScript(String scriptName, Object args, Binding binding ) throws ResourceException, ScriptException {
        ...
        Script originalScript =  _gse.createScript(filename, binding);
        attachScriptConsole(originalScript);

        return originalScript;
    }

    private void attachScriptConsole(Script originalScript) {
        // Attach this to setConsole method if available
       ((BaseGroovyScript)originalScript).setConsole(this);
